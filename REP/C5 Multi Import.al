        ...
        begin
            ...
            */
            IF Lager = TRUE THEN BEGIN
                ...
                //*  230621 - ind igen pas på længde af tekst/description01
            ...
            //her kommer sager og sagsposter - HBK / ITB - 050721
            IF Sager = TRUE THEN BEGIN
                // 3.LAGERPOSTER
                FileName := 'Projekt.csv';
                UploadIntoStream('Import Projekt', Path, '', FileName, FileInStream);
                Xmlport.Import(Xmlport::"Multi C5 Job", FileInStream);
                /*
                IF FILE.EXISTS(Path + 'Lagerposter.csv') = TRUE THEN BEGIN
                    VarXmlFile.OPEN(Path + 'Lagerposter.csv');
                    VarXmlFile.CREATEINSTREAM(VarInStream);
            ...
            IF SagsPoster = TRUE THEN BEGIN
                // 3.LAGERPOSTER
                FileName := 'ProjektPoster.csv';
                UploadIntoStream('Import Projektposter', Path, '', FileName, FileInStream);
                Xmlport.Import(Xmlport::"Multi C5 JobEntry", FileInStream);
                /*
                IF FILE.EXISTS(Path + 'Lagerposter.csv') = TRUE THEN BEGIN
                    VarXmlFile.OPEN(Path + 'Lagerposter.csv');
                    VarXmlFile.CREATEINSTREAM(VarInStream);
                ...
                */
            END;
            //her over sager og sagsopgaver/poster - 050721
